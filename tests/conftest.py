...
from hubspot_account_info import HubspotAccountInfo, DefaultAioHttpClient, AsyncHubspotAccountInfo
from hubspot_account_info._utils import is_dict
...
logging.getLogger("hubspot_account_info").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[HubspotAccountInfo]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with HubspotAccountInfo(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncHubspotAccountInfo]:
